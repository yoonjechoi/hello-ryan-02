...
- **2·3단계는 서브에이전트**(`Agent`, **model: "opus"**).
...
## Phase 2 — 디자인 시트 = 브랜드 스킬 (서브에이전트)
**스킬:** `brand-sheet` · **에이전트:** `brand-sheet-maker`
1. `Agent(subagent_type:"brand-sheet-maker", model:"opus")`로:
   회사명·슬로건 확정 → 레퍼런스 2~3 추천(참가자가 하나 고름) → 디자인 시트 추출 → **워드마크 로고**(이미지 생성 X) → **브랜드 스킬 패키징** `_workspace/brand-kit/<slug>/brand-skill/`(SKILL.md+specs+usage-examples+mark.svg).
2. 색·폰트·로고 카탈로그를 보여준다 (게이트 ②: "이 느낌 맞아요?").
3. 교육: "'이 사이트 보고 디자인 시트 만들어줘' 한 줄이면 돼요."
